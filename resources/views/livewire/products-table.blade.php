<div class="main_content">
    <div class="sidebar">
        <div class="sidebar_content">
            ...
            <a href="" onclick="window.close();return false;" class="close_btn"><i
    ...
    <div class="orders_table">
        <h2 class="table_heading">
            ...
            <div>
                ...
                <button class="table-btn add-btn" wire:click="showAddModal">Add <i
                ...
                <tbody>
                    ...
                    <tr wire:key={{$p->id}}>
                        ...
                        <td>
                            <button class="table-btn delete-btn"
                                wire:click="showDeleteModal({{ $p->id }})">delete</button>
                        </td>
                    </tr>
                    @endforeach
                    @elseif ($UsersSectionvisible)
                    ...
                    <tr wire:key={{$u->id}}>
                        ...
                        <td>
                            ...
                            <button class="table-btn view-btn" wire:click="showConfirmModal({{ $u->id }})">make
                                admin</button>
                            <button class="table-btn delete-btn"
                                wire:click="showDeleteModal({{ $u->id }})">delete</button>
                            @else
                    ...
                    <tr wire:key={{$c->id}}>
                        ...
                        <td>
                            <button class="table-btn edit-btn"
                                wire:click="showCategoryEditModal({{ $c->id }})">edit</button>
                        </td>
                        <td>
                            <button class="table-btn delete-btn"
                                wire:click="showDeleteModal({{ $c->id }})">delete</button>
                        </td>
                    </tr>
                    @endforeach
                    @endif
        ...
        <div class="modal_wrapper">
            <div class="modal-content">
                ...
                @endphp
                @if ($productSectionvisible)
                <form action=" {{ route('addProduct') }} " class="modal-form" enctype="multipart/form-data"
                    method="POST">
                    @csrf
                    <div>
                        <label for="name">Name</label>
                        <input type="text" name="name" class="modal-form-input @error('name') input-error @enderror"
                    ...
                    <div>
                        ...
                        <select name="category_id" wire:model="category_id" class="modal-form-input">
                    ...
                    <div>
                        ...
                        <textarea name="description" wire:model="description" id="modal-textarea"
                            class="modal-form-input @error('description') input-error @enderror"></textarea>
                        <span class="input-error-msg">
                    ...
                    <div>
                        ...
                        <input type="number" name="price" class="modal-form-input @error('price') input-error @enderror"
                            autocomplete="off" wire:model="price" min="0" oninput="validity.valid||(value='');">
                        <span class="input-error-msg">
                    ...
                    <div class="flex-btns">
                        <input type="submit" class="table-btn {{ $isEditModalOpen ? 'edit-btn' : 'add-btn' }}"
                            value="{{ $isEditModalOpen ? 'Update' : 'Add' }}">
                ...
                <form wire:submit.prevent=" {{$action}} " class="modal-form">
                    ...
                    <div>
                        <label for="description">Description</label>
                        <textarea name="" id="modal-textarea" wire:model.defer="description"
                            class="modal-form-input @error('description') input-error @enderror"
                    ...
                    <div class="flex-btns">
                        <button type="submit" class="table-btn {{ $isEditModalOpen ? 'edit-btn' : 'add-btn' }}">{{
                            $isEditModalOpen ? 'Update' : 'Add' }}</button>
                        <button type="button" wire:click="closeModal" class="table-btn close_btn">Cancel</button>
                    </div>
                    @endif
        ...
        <div class="modal_wrapper">
            <div class="modal-content">
                ...
                <p class="modal-text">Are you sure you want to {{$isDeleteModalOpen ? 'delete this record?' : 'make this
                    user an admin?'}}</p>
                <div class="flex-btns">
                    @php
                    $action = '';
